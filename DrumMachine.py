...
def draw_load_menu(index):
    ...
    if 0 <= index < len(my_beats):
        pygame.draw.rect(screen, light_gray, [190, 100 + index*50, 1000, 50])
    for beat in range(len(my_beats)):
        if beat < 10:
            beat_clicked = []
            row_text = medium_font.render(f'{beat + 1}', True, white)
            screen.blit(row_text, (200, 100 + beat * 50))
            name_index_start = my_beats[beat].index('name: ') + 6
            name_index_end = my_beats[beat].index(', beats:')
            name_text = medium_font.render(my_beats[beat][name_index_start:name_index_end], True, white)
            screen.blit(name_text, (240, 100 + beat * 50))
        if 0 <= index < len(my_beats) and beat == index:
            beat_index_end = my_beats[beat].index(', bpm')
            loaded_beats = int(my_beats[beat][name_index_end + 8: beat_index_end])
            bpm_indext_end = my_beats[beat].index(', selected:')
            loaded_bpm = int(my_beats[beat][beat_index_end + 6: bpm_index_end])
            loaded_clicks_string = my_beats[beat][bpm_indext_end + 14: -3]
...
while run:
    ...
    # bpm
    bpm_rect = pygame.draw.rect(screen, gray, [306, HEIGHT - 150, 200, 100], 5, 5)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONUP and not save_menu and not load_menu:
            ...
            if exit_button.collidepoint(event.pos):
                ...
                for i in range(len(my_beats)):
                    file.write(str(my_beats[i]))
                file.close()
                svae_menu = False
                typing = False
                beat_name = ''
